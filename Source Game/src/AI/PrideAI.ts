...
export default class PrideAI extends StateMachineAI implements BattlerAI {
    ...
    handleEvent(event: GameEvent): void {
        if (event.type === Game_Events.PRIDE_ENVY_PUNCH) {
            this.punch.use(this.owner, "enemies", this.owner.position.dirTo(this.player.position));
        }
        else if(event.type === Game_Events.PRIDE_GLUT_ATTACK) {
        ...
        else if(event.type === Game_Events.PRIDE_GREED_ATTACK) {
        ...
        else if(event.type === Game_Events.PRIDE_WRATH_ATTACK_UP) {
            this.slice.use(this.owner, "enemies", Vec2.UP);
            this.changeState(BossStates.WRATH_ATTACK_UP);
        }
        else if (event.type === Game_Events.PRIDE_WRATH_ATTACK_DOWN) {
